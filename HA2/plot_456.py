...
ax_cutoff.plot(energy_cutoff[2:], sigma_2[2:])
ax_cutoff.set_ylabel('Simulated surface energy density [eV/$Å^2$]', size=12)
ax_cutoff.set_xlabel('Cutoff energy [atomic units]')
ax_cutoff.set_xlim([energy_cutoff[2], energy_cutoff[-1]])
...
ax_Nz.set_xlabel('Atom depth in surface slab')
ax_Nz.set_xlim([Nz[0], Nz[-1]])
ax_Nz.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
fig.subplots_adjust(hspace=0.5)
plt.savefig('converge_surface.eps', bbox_inches='tight')
plt.show()
