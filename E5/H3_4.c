...
double update_walker_2_guided(double * r_walker1, double *r_walker2, int axis, double timestep, double guidance_force, gsl_rng *q);
...
int main() {
  ...
  double timestep = 0.01; /* Initial time step, should be a value between 0.01
                             and 0.1 */
  double alpha = 0.05; /* Constant for energy update algorithm, should be a value
                         between 0 and 1 */
  ...
  /*Variables specific to Metropolis timestep correction of Force Biased Monte Carlo */
  double guidance_force_prev; /* Guidance step before (potential) update */
  double guidance_force_temp; /* Guidance step after (potential) update */
  double transition_prev_temp; /* Transition matrix element from timestep k to k + 1*/
  double transition_temp_prev; /* Transition matrix element from timestep k + 1 to k */
  double alpha_prev_temp; /* Transition probability */

  int sum_acc_tot_r1 = 0; /* Number of accepted timesteps for r1 */
  int sum_dec_tot_r1 = 0; /* Number of declined timesteps for r1 */
  int sum_acc_tot_r2 = 0; /* Number of accepted timesteps for r2 */
  int sum_dec_tot_r2 = 0; /* Number of declined timesteps for r2 */
  ...
  file_walk_pos1 = fopen("walk_pos1.dat","w"); /* File for storing the position of first array of walkers */
  file_walk_pos2 = fopen("walk_pos2.dat","w"); /* File for storing the position of second array of walkers */

  /* SIMULATION */
 	for(k = 1; k < nbr_of_timesteps; k++){
		/* Displace random walkers */
    counter = 0;
    potential_avg=0;
      ...
      for(l = 0; l < 3; l++){
        ...
        pos_temp2[i][l] = update_walker_2_guided(pos_prev1[i], pos_prev2[i], l, timestep, guidance_force_prev, q);

      }

      /* Accept or reject particle 1 in each direction*/
      for(l = 0; l < 3; l++){
        guidance_force_prev = calc_ext_guidance_force_walker_1(pos_prev1[i], pos_prev2[i], l);
        guidance_force_temp = calc_ext_guidance_force_walker_1(pos_temp1[i], pos_temp2[i], l);

        transition_prev_temp = calc_transition_matrix(pos_prev1[i][l], pos_temp1[i][l], timestep, guidance_force_prev);
        transition_temp_prev = calc_transition_matrix(pos_temp1[i][l], pos_prev1[i][l], timestep, guidance_force_temp);

        alpha_prev_temp = (pow(psi_Test(pos_temp1[i], pos_prev2[i]), 2.0)*transition_temp_prev)/
                          (pow(psi_Test(pos_prev1[i], pos_prev2[i]), 2.0)*transition_prev_temp);

        if(alpha_prev_temp > 1.0){
          alpha_prev_temp = 1;
        }
        if (alpha_prev_temp >= gsl_rng_uniform(q)){
  				sum_acc_tot_r1+=1;
  			}
  			else{
  				pos_temp1[i][l] = pos_prev1[i][l];
  				sum_dec_tot_r1+=1;
  			}
      }

      /* Accept or reject particle 2 in each direction*/
      for(l = 0; l < 3; l++){
        guidance_force_prev = calc_ext_guidance_force_walker_2(pos_prev1[i], pos_prev2[i], l);
        guidance_force_temp = calc_ext_guidance_force_walker_2(pos_temp1[i], pos_temp2[i], l);

        transition_prev_temp = calc_transition_matrix(pos_prev2[i][l], pos_temp2[i][l], timestep, guidance_force_prev);
        transition_temp_prev = calc_transition_matrix(pos_temp2[i][l], pos_prev2[i][l], timestep, guidance_force_temp);

        alpha_prev_temp = (pow(psi_Test(pos_prev1[i], pos_temp2[i]), 2.0)*transition_temp_prev)/
                          (pow(psi_Test(pos_prev1[i], pos_prev2[i]), 2.0)*transition_prev_temp);
...
printf("%s%e\n","Acceptence quota r1", (double)sum_acc_tot_r1/
                                       (double)(sum_acc_tot_r1+sum_dec_tot_r1));
printf("%s%e\n","Acceptence quota r2", (double)sum_acc_tot_r2/
                                       (double)(sum_acc_tot_r2+sum_dec_tot_r2));
...
double V_x(double x){
  /* Function that calculates harmonic potential at x, given k = 1 */
  return 0.5*x*x;
}

double V_helium(double x1, double y1, double z1, double x2, double y2, double z2){
  /* Function that calculates the potential energy for the helium atom*/
...
double update_walker_1_guided(double * r_walker1, double *r_walker2, int axis, double timestep, double guidance_force, gsl_rng *q){
  return r_walker1[axis] +
         timestep/2.0*guidance_force +
         sqrt(timestep)*gsl_ran_gaussian_ratio_method(q,1);
}

double update_walker_2_guided(double * r_walker1, double *r_walker2, int axis, double timestep, double guidance_force, gsl_rng *q){
    return r_walker2[axis] +
         timestep/2.0*guidance_force +
         sqrt(timestep)*gsl_ran_gaussian_ratio_method(q,1);
};
...
double calc_transition_matrix(double x_prev, double x_temp, double timestep, double external_force_prev){
  return exp(-(1.0/(2.0*timestep))*pow( (x_temp - x_prev - (timestep/(2.0)*external_force_prev)), 2.0));
}
